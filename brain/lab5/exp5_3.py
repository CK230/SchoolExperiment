...
def add_salt_pepper_spike_noise(spike_times, noise_level, rng):
    # TODO 3：补全椒盐型脉冲噪声逻辑
    # noise_level 范围 0~30，映射到概率 p = noise_level / 100.0
    p = noise_level / 100.0
    # 胡椒：随机删除
    kept = [t for t in spike_times if rng.random() >= p]
    # 盐：随机注入一个新脉冲
    if rng.random() < p:
        # 注入的脉冲时刻在 [0, T_WINDOW-1] 内均匀随机
        new_spike = rng.integers(0, T_WINDOW)
        kept.append(new_spike)
    return kept


# 根据频率编码构造理想脉冲时刻，再施加“椒盐型脉冲噪声 + 高斯时间抖动”
def process_rate_coding(image, noise_level):
    # TODO 4：补全频率编码的编码-加噪-重构流程
    H, W = image.shape
    ...
    for i in range(H):
        for j in range(W):
            x = image[i, j]
            # 理想脉冲个数
            n = int(round(x * T_WINDOW))
            # 均匀分布脉冲时刻
            if n > 0:
                ideal_times = np.round(np.linspace(0, T_WINDOW - 1, n)).astype(int).tolist()
            else:
                ideal_times = []
            # 加椒盐噪声
            noisy_times = add_salt_pepper_spike_noise(ideal_times, noise_level, rng)
            # 加高斯时间抖动
            if noisy_times:
                noisy_times = [add_gaussian_timing_noise(t, noise_level / 10.0, rng) for t in noisy_times]
                # 裁剪到有效范围
                noisy_times = [max(0, min(T_WINDOW - 1, t)) for t in noisy_times]
            # 重构：脉冲计数 / T_WINDOW
            decoded[i, j] = len(noisy_times) / T_WINDOW
    return decoded


# 根据首次脉冲发放时间编码构造理想首次脉冲，再施加“椒盐型脉冲噪声 + 高斯时间抖动”
def process_ttfs_coding(image, noise_level):
    # TODO 5：补全 TTFS 编码的编码-加噪-重构流程
    H, W = image.shape
    decoded = np.zeros((H, W), dtype=np.float32)
    rng = np.random.default_rng()
    for i in range(H):
        for j in range(W):
            ...
            if x == 0:
                ...
                # 灰度越高，发放越早
                t = int(round(TTFS_SPAN * (1 - x)))
                t = max(0, min(T_WINDOW - 1, t))
                ideal_times = [t]
            # 加椒盐噪声
            noisy_times = add_salt_pepper_spike_noise(ideal_times, noise_level, rng)
            # 加高斯时间抖动
            if noisy_times:
                ...
                noisy_times = [max(0, min(T_WINDOW - 1, t)) for t in noisy_times]
                # TTFS 只取最早的一个脉冲
                first_time = min(noisy_times)
                # 重构：根据首次脉冲时间反推灰度
                decoded[i, j] = max(0.0, min(1.0, 1.0 - first_time / TTFS_SPAN))
            else:
                decoded[i, j] = 0.0
    return decoded
