...
def pair_based_stdp(delta_t, tau_plus=10.0, tau_minus=10.0, a_plus=0.08, a_minus=0.08):
    ...
    # delta_t > 0：pre 先于 post，LTP
    # delta_t < 0：post 先于 pre，LTD
    # delta_t == 0：近似视为无变化
    if delta_t > 0:
...
def weight_dependent_amplitude(weight, direction, mode, eta_plus, eta_minus, w_min=0.0, w_max=1.0):
    # direction == "ltp" 表示增强，使⽤ A_+(w)
    # direction == "ltd" 表示减弱，使⽤ A_-(w)
    # mode == "classic"：不考虑权重依赖
    # mode == "hard"：硬边界
    # mode == "soft"：软边界
    if mode == "classic":
        if direction == "ltp":
        ...
        if direction == "ltd":
            return eta_minus
        raise ValueError('direction 只能是 "ltp" 或 "ltd"')
    ...
    if mode == "soft":
        if direction == "ltp":
            return max(0.0, (w_max - weight) * eta_plus)
        if direction == "ltd":
            return max(0.0, (weight - w_min) * eta_minus)
        raise ValueError('direction 只能是 "ltp" 或 "ltd"')

    raise ValueError('mode 只能是 "classic"、"hard" 或 "soft"')
...
def draw_stdp_window(ax, tau_plus, tau_minus, a_plus, a_minus):
    ...
    ax.set_title("STDP 时间窗：pre 先于 post 增强，post 先于 pre 减弱")
    ax.grid(alpha=0.25)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)


# 绘制任务二：权重依赖 STDP 的时间窗形状
def draw_weight_dependent_window_panel(ax, tau_plus, tau_minus, a_plus, a_minus, w_demo, eta_plus, eta_minus, w_min=0.0, w_max=1.0):
    ...
    ax.set_xlabel(r"$\Delta t = t_{post} - t_{pre}$")
    ax.set_ylabel(r"$\Delta w$")
    ax.set_title(r"权重依赖 STDP 时间窗（本实验约定 $\Delta t=t_{post}-t_{pre}$）")
    ax.grid(alpha=0.25)
    ax.legend()
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def main():
    ...
    eta_plus = 0.12       # 权重依赖 STDP 中 LTP 的基础更新幅度
    eta_minus = 0.04      # 权重依赖 STDP 中 LTD 的基础更新幅度
