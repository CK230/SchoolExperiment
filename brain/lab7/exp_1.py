...
def decay_hebb_delta(w, v_i, v_j, gamma2, gamma0):
    # dw_ij/dt = gamma2 * (1 - w_ij) * v_i * v_j - gamma0 * w_ij
    return gamma2 * (1.0 - w) * v_i * v_j - gamma0 * w


# 门控 Hebb 学习：比较突触前门控和突触后门控
def gated_hebb_delta(v_i, v_j, eta, v_theta, gate_type):
    # gate_type == "pre"：Δw_ij = eta * (v_i - v_theta) * v_j
    # gate_type == "post"：Δw_ij = eta * v_i * (v_j - v_theta)
    if gate_type == "pre":
        return eta * (v_i - v_theta) * v_j
    if gate_type == "post":
        return eta * v_i * (v_j - v_theta)
    raise ValueError('gate_type 只能是 "pre" 或 "post"')


# 协变规则：关注活动相对平均活动水平的共同变化
def covariance_hebb_delta(v_i, v_j, v_i_bar, v_j_bar, eta):
    # Δw_ij = eta * (v_i - v_i_bar) * (v_j - v_j_bar)
...
def bounded_hebb_delta(w, v_i, v_j, bound_type, eta0=0.06, gamma2=0.06, w_max=1.0):
    # 1. bound_type == "none"：Δw_ij = eta0 * v_i * v_j；
    # 2. bound_type == "hard"：Δw_ij = eta0 * H(w_max - w) * v_i * v_j；
    # 3. bound_type == "soft"：Δw_ij = gamma2 * (w_max - w) * v_i * v_j。
    if bound_type == "none":
        return eta0 * v_i * v_j
    if bound_type == "hard":
        h = 1.0 if w < w_max else 0.0
        return eta0 * h * v_i * v_j
    if bound_type == "soft":
        return gamma2 * (w_max - w) * v_i * v_j
    raise ValueError('bound_type 只能是 "none"、"hard" 或 "soft"')


# BCM 规则：突触后活动超过阈值时增强，低于阈值时减弱
def bcm_delta(v_i, v_j, theta, eta):
    # Δw = eta * v_i * (v_i - theta) * v_j
